...
class Model extends Database{
    ...
    {
        try {
            $sql = "INSERT INTO solicitud (id, folio, estado, descripcion, departamento, trabajador, fecha_i) VALUES (null,:folio, :status, :description, :departament, :employee, NOW())";
    ...
    {
        try {
            $sql = "UPDATE solicitud SET folio=:folio, estado=:status, descripcion=:description, departamento=:departament, trabajador=:employee, fecha_fin=NOW() WHERE id=$id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':folio', $folio);
            $stmt->bindParam(':status', $status);
            $stmt->bindParam(':description', $description);
    ...
    {
        try {
            $query = $this->conn->prepare("SELECT * FROM solicitud WHERE id = $id");
            $query->execute();
